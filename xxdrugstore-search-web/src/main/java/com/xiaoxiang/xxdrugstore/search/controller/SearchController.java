package com.xiaoxiang.xxdrugstore.search.controller;

import com.alibaba.dubbo.config.annotation.Reference;
import com.xiaoxiang.xxdrugstore.annotations.LoginRequired;
import com.xiaoxiang.xxdrugstore.bean.*;
import com.xiaoxiang.xxdrugstore.service.AttrService;
import com.xiaoxiang.xxdrugstore.service.SearchService;
...
public class SearchController {
    ...
    public String list(HttpServletRequest request,PmsSearchParam pmsSearchParam, ModelMap modelMap) {//三级分类id，关键字
        ...
        if (delValueIds != null) {
            ...
            for (String delValueId : delValueIds) {
                Iterator<PmsBaseAttrInfo> iterator = pmsBaseAttrInfos.iterator();//迭代器只能循环一次，所以每次循环都要new一个迭代器，所以这行代码放在52行后，而不是之前
    ...
    private String getUrlParam(PmsSearchParam pmsSearchParam) {
        ...
        if (StringUtils.isNotBlank(keyword)) {
            if (StringUtils.isNotBlank(urlParam)) {//第一个字符不需要&,后面的才要，所以先判断是否为空
                urlParam = urlParam + "&";
            }
            urlParam = urlParam + "keyword=" + keyword;
        }
