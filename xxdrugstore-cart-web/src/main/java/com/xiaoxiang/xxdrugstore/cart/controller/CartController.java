package com.xiaoxiang.xxdrugstore.cart.controller;
...
import com.xiaoxiang.xxdrugstore.util.CookieUtil;
...
public class CartController {
    ...
    @RequestMapping("checkCart2")
    @LoginRequired(loginSuccess = false)  //此注解为自己编写的，含义是方法是否需要登录验证，括号内的含义是是否一定需要登录成功
    public String checkCart2(String quantity, String skuId, HttpServletRequest request, HttpServletResponse response, HttpSession session, ModelMap modelMap) {
        String memberId = (String) request.getAttribute("memberId");
        String nickname = (String) request.getAttribute("nickname");
        //调用服务，修改状态
        OmsCartItem omsCartItem = new OmsCartItem();
